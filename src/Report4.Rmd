...
incomeDf = CreditCard[c("income", "card")]

# Calculate Q1 and Q3
Q1 <- quantile(incomeDf$income, 0.25)
Q3 <- quantile(incomeDf$income, 0.75)
IQR <- Q3 - Q1
...
incomeDf <- incomeDf[incomeDf$income >= lower_bound & incomeDf$income <= upper_bound, ]

hist(incomeData$income, main = "Histogram of People's Income", xlab = "Income $10K", breaks = "Sturges", col = "lightblue")
```

```{r}
ggplot(incomeData, aes(x=income, color=card)) + 
  geom_histogram(fill="white", bins=40)
```


```{r}
yesVec = na.omit(as.numeric(incomeData[incomeData["card"] == "yes"]))
noVec = na.omit(as.numeric(incomeData[incomeData["card"] == "no"]))

incomeTest = wilcox.test(yesVec, noVec, exact = FALSE)
print(incomeTest)
```
...
library(tidyr)
values1 = c(1, 2, 2, 2, 2, 1, 1, 2, 3, 4, 5, 6,2 ,3 ,4 ,5, 3, 3, 3, 3, 4, 1, 2, 2)
values2 = c(7, 8, 6, 6 ,5 ,5 ,6 ,3, 3, 4, 5 ,5 ,7 ,8 ,8 ,7 ,6 ,8 ,9, 9,0, 9,6 , 5)

df <- data.frame(values1, values2)

df_long <- pivot_longer(df, 
                        cols = c(values1, values2), 
                        names_to = "variable", 
                        values_to = "value")
...
```{r}
incomeData$income = as.numeric(incomeData$income)
incomeData = na.omit(incomeData)

# Convert the binary outcome to a factor
incomeData$binary_outcome <- as.factor(incomeData$card)

# Fit the logistic regression model
model <- glm(binary_outcome ~ income, data = incomeData, family = binomial())
