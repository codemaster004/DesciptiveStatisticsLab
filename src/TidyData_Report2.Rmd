...
colSums(is.na(sample_dirty_dataset))

# Check for unique values in categorical variables
unique_values <- lapply(sample_dirty_dataset, unique)
print(unique_values)
```

```{r}
# Correct errors in age column
sample_dirty_dataset$age <- as.numeric(ifelse(sample_dirty_dataset$age == "InvalidAge", NA, sample_dirty_dataset$age))

# Correct errors in capital_gain column
sample_dirty_dataset$capital_gain <- ifelse(sample_dirty_dataset$capital_gain < 0, 0, sample_dirty_dataset$capital_gain)

# Correct errors in capital_loss column
sample_dirty_dataset$capital_loss <- ifelse(sample_dirty_dataset$capital_loss < 0, 0, sample_dirty_dataset$capital_loss)

# Correct errors in occupation column
...
profile <- dlookr::dlk_profile(sample_dirty_dataset)
